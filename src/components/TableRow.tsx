...
export default function TableRow({ subject }: Props) {
  ...
  return (
    <tr
      className="bg-color3 text-color1 border-b border-color2 flex h-14 hover:cursor-pointer hover:bg-color4 transition-all group text-lg font-semibold"
      key={subject._id}
      ref={fatherRef}
    ...
    >
      ...
      <td className="relative w-[10%] h-full flex items-center justify-center z-40">
        ...
        <svg
          ...
          height="16"
          fill="currentColor"
          className="bi bi-x-circle-fill absolute top-1.5 right-1.5 size-3 text-color2 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100"
          viewBox="0 0 16 16"
          onClick={(e) => {
